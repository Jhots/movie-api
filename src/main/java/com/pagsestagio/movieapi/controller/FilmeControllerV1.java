...
public class FilmeControllerV1 {
  ...
  public ResponseEntity<FilmeResposta> pegarFilmePeloId(@PathVariable Integer id) {
    ...
    if (retornoService.mensagemStatus() == null) {
      respostaRequisicao =
          ResponseEntity.ok()
              .body(
                  new FilmeRespostaRetornaFilmeOuExcecaoV1(
                      retornoService.id(), retornoService.nomeFilme(), null));
    } else {
      respostaRequisicao =
  ...
  public ResponseEntity<FilmeResposta> criarFilme(@RequestBody FilmeDTOV1 filme) {
    ...
    if (retornoService.mensagemStatus() == null) {
      respostaRequisicao =
          ResponseEntity.ok()
              .body(
                  new FilmeRespostaRetornaListaDeFilmesOuExcecaoV1(
                      retornoService.listaDeFilmes(), null));
    } else {
      respostaRequisicao =
          ResponseEntity.badRequest()
              .body(
                  new FilmeRespostaRetornaListaDeFilmesOuExcecaoV1(
                      null, retornoService.mensagemStatus()));
    }
    return respostaRequisicao;
  }
  ...
  public ResponseEntity<FilmeResposta> atualizarFilme(@RequestBody FilmeDTOV1 filme) {
    FilmeResultadoRetornaListaDeFilmesOuExcecaoV1 retornoService =
        filmeService.atualizarFilmeV1(filme);
    ResponseEntity<FilmeResposta> respostaRequisicao = null;
    if (retornoService.mensagemStatus() == null) {
      respostaRequisicao =
          ResponseEntity.ok()
  ...
  public ResponseEntity<FilmeResposta> deletarFilmePeloId(@PathVariable Integer id) {
    FilmeResultadoRetornaListaDeFilmesOuExcecaoV1 retornoService =
        filmeService.deletarFilmePeloIdV1(id);
    ResponseEntity<FilmeResposta> respostaRequisicao = null;
